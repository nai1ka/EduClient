...
class DiaryFragmentOneDayPerPage : IDiaryFragment(), SharedPreferences.OnSharedPreferenceChangeListener,
            ...
            .observe(
                ...
                if (payload.first.rawArray.size > 0) {
                    ...
                    CurrentDiaryState.additionalSize =
                        payload.first.rawArray.size - payload.first.rawArray.size
                    pagerAdapter.setItems(payload.first.rawArray)
                    ...
                    if (isFirstLaunch || CurrentDiaryState.isTabLayoutNeedsToBeUpdated) {
                        ...
                        vp.setCurrentItem(daysBetween, false)
                        isFirstLaunch = false
    ...
    fun attachMediator(payloadData: ArrayList<Day>) {
        TabLayoutMediator(tabLayout, vp, false) { tab, position ->
            ...
            tab.text =
                "${payloadData[position].date} ${
                    Utils.getCorrectMonthTitle(
                        payloadData[position].month
                    )
    ...
    override fun setToToday() {
        vp.setCurrentItem(daysBetween , true)
    }
    ...
    private inner class ScreenSlidePagerAdapter(f: Fragment) : FragmentStateAdapter(f) {
        private var subListItems: ArrayList<Day> = arrayListOf()
        ...
        fun setItems(newItems: ArrayList<Day>) {
